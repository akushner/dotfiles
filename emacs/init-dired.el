...
;; $Id: init-dired.el,v 1.14 2007/05/30 20:29:51 friedman Exp $
...
(defun idired-mode-setup ()
  ...
  (cond ((eq (emacs-variant) 'emacs)
         ...
         (setq font-lock-defaults '(idired-font-lock-keywords t))))
...
(defadvice efs-ls (before remove-incompat-switches activate)
  "Remove switches from args to `ls' that remote ftp servers are unlikely to handle."
  (let* ((switches (or (ad-get-arg 1) (efs-ls-guess-switches))))
    (cond ((stringp switches)
           (setq switches (mapcar 'identity switches))
           (mapc (lambda (c) (setq switches (delete c switches))) "sbv")
           (ad-set-arg 1 (mapconcat 'char-to-string switches ""))))))
...
;; This variable is in emacs's standard dired, but not in efs-dired;
;; efs-dired uses the value of dired-no-confirm instead.
