...
;; Copyright (C) 1994, 95, 96, 97, 98, 99, 2000 Noah S. Friedman

;; Author: Noah Friedman <friedman@splode.com>
;; Maintainer: friedman@splode.com

;; $Id: init-vm.el,v 1.125 2009/05/27 21:40:32 friedman Exp $
...
;; It might or might not work with older versions, but I haven't tested.
;(and (string-lessp vm-version "6.72")
;     (error "VM version %s is too old to work with vm-init.el" vm-version))
...
(defun ivm-vm-presentation-mode-initialize ()
...
(defun ivm-vm-summary-mode-initialize ()
  (ivm-set-mode-line-format))
...
(setq vm-reply-ignored-addresses
      (list (vm-multdom-address-list-regexp vm-multdom-user-addresses)))
...
(setq vm-resend-bounced-discard-header-regexp
      (vma-globlist->regexp '("Message-Id"
                              "Received"
                              "Return-Path"
                              "Sender")))
...
(add-hook 'vm-mode-hook 'ivm-vm-mode-initialize)
(add-hook 'vm-presentation-mode-hook 'ivm-vm-presentation-mode-initialize)
(add-hook 'vm-summary-mode-hook 'ivm-vm-summary-mode-initialize)
...
(add-forms-to-after-load-alist
  '(("bbdb"
    ...
    ("u-vm-color"
     ...
     (mapc (lambda (elt)
             (override-face-attributes (car elt) '(t)
               :inherit (cadr elt)))
       '((u-vm-color-default-face       default)

         ;; headers and header contents
         (u-vm-color-header-face        font-lock-preprocessor-face)
         (u-vm-color-author-face        font-lock-function-name-face)
         (u-vm-color-recipient-face     u-vm-color-default-face)
         (u-vm-color-subject-face       font-lock-function-name-face)
         (u-vm-color-date-face          u-vm-color-default-face)
         (u-vm-color-time-face          u-vm-color-default-face)
         ...
         (u-vm-color-citation-1-face    font-lock-builtin-face)
         (u-vm-color-citation-2-face    font-lock-string-face)
         (u-vm-color-citation-3-face    font-lock-function-name-face)
         (u-vm-color-citation-4-face    font-lock-comment-face)
         (u-vm-color-citation-5-face    font-lock-type-face))))
    ...
    ("vm-addons"
     ...
     (add-hook 'vm-arrived-messages-hook
               'vma-kill-duplicate-messages-by-message-id)
